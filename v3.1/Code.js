 ...
 * @throws {Error} If an error occurs while retrieving the course list.
 */
function courseData(teacherId) {
  const options = {
    teacherId: teacherId,
    courseStates: 'ACTIVE'
  };
  ...
  try {
    const courseList = Classroom.Courses.list(options);
    if (courseList && Array.isArray(courseList.courses)) {
      const course = courseList.courses;
      course.forEach((courseItem) => {
        Logger.log(`course name: ${courseItem.name}, course ID: ${courseItem.id}`);
      });
    }
  } catch (error) {
...
function onOpen() {
  ...
  ui.createMenu('Extract')
      .addItem('get class data', 'insertData')
      .addItem('get submission state', 'checkData')
      .addToUi();
}
